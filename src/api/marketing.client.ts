...
export const marketingClient = {
  ...
  schedulePost: async (eventId: IdParam, data: SchedulePostDto) => {
    return await httpApi.post(`/planner/events/${eventId}/marketing/facebook/schedule`, data);
  },

  getFacebookPosts: async (eventId: IdParam, pageId: string) => {
    const response = await httpApi.get<FacebookPostsResponse>(`/planner/events/${eventId}/marketing/facebook/posts`, {
      params: { pageId },
    });
    return response.data;
  },
  ...
  checkFacebookAuth(userId: string) {
    const response = httpApi.get<{ isAuthenticated: boolean }>(`/auth/facebook/check`, {
      params: { userId },
    });
    return response;
  },
  ...
  disconnectFacebook(userId: string) {
    const response = httpApi.post<{ success: boolean }>(`/auth/facebook/disconnect`, null, {
      params: { userId },
    });
    return response;
  },
