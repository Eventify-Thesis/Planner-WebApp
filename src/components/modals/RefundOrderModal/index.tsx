...
export const RefundOrderModal = ({
  ...
  const handleSubmit = (values: RefundOrderPayload) =>
    mutation.mutate(
      {
        eventId: eventId,
        orderId: orderId,
  ...
  const modalForm = ({
    ...
    order: Order;
    form: UseFormReturnType<RefundOrderPayload>;
  }) => {
    return (
      <form onSubmit={form.onSubmit(handleSubmit)}>
        <div className={classes.refundSummary}>
          ...
          <div className={classes.block}>
            <div className={classes.title}>{t`Total refunded`}</div>
            <div className={classes.amount}>
              <Currency
          ...
          <div className={classes.block}>
            <div className={classes.title}>{t`Total remaining`}</div>
            <div className={classes.amount}>
              <Currency
                currency={order.currency}
  ...
  const getModalBody = () => {
    if (order.is_manually_created) {
      return (
        <CannotRefund
          message={t`You cannot refund a manually created order.`}
        />
      );
    }
    ...
    if (isRefunded) {
      return (
        <CannotRefund message={t`This order has already been refunded.`} />
      );
    }
